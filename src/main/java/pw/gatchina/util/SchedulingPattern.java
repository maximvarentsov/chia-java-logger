...
public class SchedulingPattern {
     ...
     * false otherwise.
     */
    public static boolean validate(final @NotNull String schedulingPattern) {
        try {
     ...
     */
    public SchedulingPattern(final @NotNull String pattern) throws InvalidPatternException {
        this.asString = pattern;
        var st1 = new StringTokenizer(pattern, "|");
        if (st1.countTokens() < 1) {
            throw new InvalidPatternException("invalid pattern: \"" + pattern + "\"");
        }
        while (st1.hasMoreTokens()) {
            var localPattern = st1.nextToken();
            var st2 = new StringTokenizer(localPattern, " \t");
     ...
     * @throws Exception If the supplied pattern part is not valid.
     */
    private List<Integer> parseListElement(final @NotNull String str, final @NotNull ValueParser parser) throws Exception {
        var st = new StringTokenizer(str, "/");
     ...
     * @throws Exception If the supplied pattern part is not valid.
     */
    private List<Integer> parseRange(final @NotNull String str, final @NotNull ValueParser parser) throws Exception {
        if (str.equals("*")) {
     ...
     */
    public long next(final @NotNull TimeZone timezone, final long millis) {
        long next = -1L;

        var gc = new GregorianCalendar(timezone);

        for (var i = 0; i < matcherSize; i++) {
            gc.setTimeInMillis(millis);
            // Go a minute ahead.
            ...
            for (;;) {
                ...
                var isLeapYear = gc.isLeapYear(year);

                for (var month = gc.get(Calendar.MONTH) + 1; month <= MONTH_MAX_VALUE; month++) {
                    if (monthMatcher.match(month)) {
                        gc.set(Calendar.MONTH, month - 1);
                        var maxDayOfMonth = DayOfMonthValueMatcher.getLastDayOfMonth(month, isLeapYear);
                        for (var dayOfMonth = gc.get(Calendar.DAY_OF_MONTH); dayOfMonth <= maxDayOfMonth; dayOfMonth++) {
                            if ((dayOfMonthMatcher instanceof DayOfMonthValueMatcher) ? ((DayOfMonthValueMatcher) dayOfMonthMatcher).match(dayOfMonth, month, isLeapYear) : dayOfMonthMatcher.match(dayOfMonth)) {
                                ...
                                if (dayOfWeekMatcher.match(dayOfWeek)) {
                                    for (var hour = gc.get(Calendar.HOUR_OF_DAY); hour <= HOUR_MAX_VALUE; hour++) {
     ...
     */
    private static int parseAlias(final @NotNull String value, final @NotNull String[] aliases, int offset) throws Exception {
    ...
    private interface ValueParser {
         ...
         */
        int parse(final @NotNull String value) throws Exception;
        /**
    ...
    private static class IntArrayValueMatcher implements ValueMatcher {
         ...
         */
        public IntArrayValueMatcher(final @NotNull List<Integer> integers) {
